...
SDカード用のSPI 4本は専用に割り当てている。他の信号と共用していない。

SPI 4本のうちSSを除く3本をデータバスと兼用して動作させることができているという報告が上がっている。Z80実行中はSSをネゲート(Hにする)しておき、MISOをHi-Zにすることで動作するとのことである。私はこのアプローチを採用せず、SPI 4本とも別に確保して割り当てている。

SDカードはSSをHにした状態ならばSCLK, MISOを任意にON/OFFしてもよいわけではない。SS\==HでSCLKに72パルスを加えるとSDカードはリセット状態に入り、初期化シーケンスを構成する特定のコマンドしか受け付けなくなるとされている。なので、SCLKとデータバス/アドレスバス信号を兼用すべきではない。

実際には兼用しても問題なく動作するようなので、72パルスも一定時間内とかそういう追加の条件があるのかもしれない。このあたりはSDカード側の実装にも依存するだろう。

一方、SDカードアダプタの電圧レベル変換に74HC125を用いている場合、SS\==H の時にSCLKwを切り離すことで期待しないリセット条件発生を抑止することができることも確かである。ただし、今度は、SDカードにリセットを掛けたいときに実現できなくなる。
SS\==Hの時にSCLKを駆動するために、PICのGPIOピンをさらに一つ割り当てる必要がでてくる。結局、共有により節約できるのは2本だけとなる。

後述の通り、アドレスバスに割り当てるピンを減らすことで余裕があるので、SDカード用に4本割り当てることも可能となった。そこまでして節約する必要もないので、SDカード用に4本割り当てている。
...
2はかの名作Z80-MBC2が採用している方法である。命令置きの場合、SRAMに7バイトを展開する必要もない。PICはアドレスバスを使う必要がなく、I/Oアドレス用途以外にアドレスピンをつないでおく必要もない。Z80-MBC2では徹底していて、PICにはアドレスバス1本(A0)しかつながっていない。この方式で試作したという報告も上がっている。

## 間接接続

Z80とSRAMの間の信号OE/, WE/ をZ80 のRD/, WR/と直結せずに、PICにOE/, WE/をつなぐ方法もある。PICはCLCを用いて、Z80のMREQ/, RD/からOE/, WE/を発生する。

このやり方は奥江さんに教えてもらいました。この方法は、Z80がプログラム実行中もメモリアクセスを検出して待たせることができるので、いろいろ仕込みをいれる可能性があって楽しいのです。

しかし、この方法ではSRAMのOE/, WE/がZ80がRD/を出してから50ns遅延することが分かった。実際に16MHzでは動作しない、SRAM OE/が間に合っていなかった。SRAMがデータを出力し始める時点でZ80はリードサイクルを終えてしまっていたのだ。

クロック8MHz以下なら確実に動作するので、他のCPUをつないで動かすときには有益だろう。ということで、関節接続に切り替えることができるようにジャンパパターンを入れておいた(JP1,2,3)
